...
1. Роль: cluster-admin  
   - Права роли: Полный доступ ко всем ресурсам в кластере.  
   - Группы пользователей: system:masters (например, для администраторов платформы Kubernetes).  

2. Роль: namespace-reader  
   - Права роли: Доступ только на чтение ресурсов в конкретном namespace. Может просматривать Deployment, Pod, Service и другие ресурсы, но не изменять их.  
   - Группы пользователей: developers, support (для отладки и мониторинга).  

3. Роль: namespace-developer  
   - Права роли: Доступ на чтение, создание, обновление и удаление ресурсов в конкретном namespace. Может создавать и редактировать Deployment, Pod, Service и другие ресурсы.  
   - Группы пользователей: developers.  

4. Роль: secret-viewer  
   - Права роли: Доступ только на чтение секретов (Secrets) во всех namespaces. Для пользователей, которым нужно знать конфигурационные данные, но не менять их.  
   - Группы пользователей: security, auditors.  

5. Роль: resource-quota-manager  
   - Права роли: Доступ на управление ResourceQuota в конкретном namespace. Для управления ограничениями ресурсов.  
   - Группы пользователей: platform-engineering.  
